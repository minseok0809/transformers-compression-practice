...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "The log directory where the model predictions and checkpoints will be written."
    ) # (revised)
    ...
    parser.add_argument("-b", "--do_evaluation", 
                        type=str2bool,
                        default=False, 
                        # action='store_false',
                        help="Whether to run eval on the dev set.") # (revised)
    ...
    # (revised)
    task_type1 = ["sst-2", "mnli", "mnli-mm",
                    "qnli", "rte", "wnli"]
    task_type2 = ["cola"]
    task_type3 = ["mrpc", "qqp"]
    task_type4 = ["sts-b"]

    first_datetime = str(datetime.datetime.strptime("0:00:00", '%H:%M:%S').time())

    if any(task_name == task_type for task_type in task_type1):
        student_evaluation_log_df = pd.DataFrame({'Epoch':[0], 'Runtime':[first_datetime], 'Accuracy':[0.0],
                                                  'Cls Loss':[0.0], 'Value Loss':[0.0],'Query Loss':[0.0], 'Key Loss':[0.0], 
                                                  'Rep Loss':[0.0], 'Loss':[0.0]})
    
    elif any(task_name == task_type for task_type in task_type2):        
        student_evaluation_log_df = pd.DataFrame({'Epoch':[0], 'Runtime':[first_datetime], 'Accuracy':[0.0],
                                                  'Cls Loss':[0.0], 'Value Loss':[0.0],'Query Loss':[0.0], 'Key Loss':[0.0], 
                                                  'Rep Loss':[0.0], 'Loss':[0.0]})

    elif any(task_name == task_type for task_type in task_type3):        
        student_evaluation_log_df = pd.DataFrame({'Epoch':[0], 'Runtime':[first_datetime], 'Accuracy':[0.0], 'F1':[0.0], 'Accuracy & F1':[0.0],
                                                  'Cls Loss':[0.0], 'Value Loss':[0.0],'Query Loss':[0.0], 'Key Loss':[0.0], 
                                                  'Rep Loss':[0.0], 'Loss':[0.0]})

    elif any(task_name == task_type for task_type in task_type4):
        student_evaluation_log_df = pd.DataFrame({'Epoch':[0], 'Runtime':[first_datetime], 'Pearson':[0.0], 'Spearmanr':[0.0], 
                                                'Pearson & Spearmanr':[0.0], 'Correlation':[0.0],
                                                'Cls Loss':[0.0], 'Value Loss':[0.0],'Query Loss':[0.0], 'Key Loss':[0.0], 
                                                'Rep Loss':[0.0], 'Loss':[0.0]})
    ...
    teacher_param_size = 0
    teacher_buffer_size = 0
    for key, val in teacher_model.state_dict().items():
        teacher_param_size += val.nelement() * val.element_size()

    for buffer in teacher_model.buffers():
        teacher_buffer_size += buffer.nelement() * buffer.element_size()

    teacher_size_all_mb = (teacher_param_size + teacher_buffer_size) / 1024**2
    print()
    ...
    if any(task_name == task_type for task_type in task_type1):
        fp32_acc = result["acc"]    
        print("Teacher Model:", task_name)
        print("Acuuracy:", fp32_acc)
        print()
        with open(args.log_dir + "_teacher_evaluation_log.txt", 'w', encoding='utf-8') as f:
            f.write(task_name + "\n")
            f.write("Acuuracy: " + str(fp32_acc))
        ...
        with open(args.log_dir + "_teacher_evaluation_log.txt", 'w', encoding='utf-8') as f:
            f.write(task_name + "\n")
            f.write("mcc: " + str(fp32_mcc))

    elif any(task_name == task_type for task_type in task_type3):        
        fp32_acc = result["acc"]
        fp32_f1 = result["f1"]
        fp32_acc_and_f1 = result["acc_and_f1"]
        print("Teacher Model:", task_name)
        print("acc:", fp32_acc)
        print("F1:", fp32_f1)
        print("Accuracy & F1:", fp32_acc_and_f1)
        print()
        with open(args.log_dir + "_teacher_evaluation_log.txt", 'w', encoding='utf-8') as f:
            f.write(task_name)
            f.write("Acuuracy: " + str(fp32_acc) + "\n")
            f.write("F1: " + str(fp32_f1) + "\n")
            f.write("Accuracy & F1: " + str(fp32_acc_and_f1))

    elif any(task_name == task_type for task_type in task_type4):
        fp32_pearson = result["pearson"]
        fp32_spearmanr = result["spearmanr"]
        fp32_pearson_and_spearmanr = result["pearson"]/result["spearmanr"]
        fp32_corr = result["corr"]
        print("Teacher Model:", task_name)
        print("Pearson:", fp32_pearson)
        print("Spearmanr:", fp32_spearmanr)
        print("Pearson & Spearmanr:", fp32_pearson_and_spearmanr)        
        print("Correlation:", fp32_corr) 
        print()
        with open(args.log_dir + "_teacher_evaluation_log.txt", 'w', encoding='utf-8') as f:
            f.write(task_name + "\n")
            f.write("Pearson: " + str(fp32_pearson) + "\n")
            f.write("Spearmanr: " + str(fp32_spearmanr) + "\n")
            f.write("Pearson & Spearmanr: " + str(fp32_pearson_and_spearmanr) + "\n")    
            f.write("Correlation: " + str(fp32_pearson_and_spearmanr))    
    # (revised)
    ...
    if args.do_evaluation == True:
        ...
        for epoch_ in range(int(args.num_train_epochs)):
            ...
            train_pbar.set_description("Epoch " + str(epoch_ + 1))
                ...
                #    logger.info(f"  Previous best {previous_best}")
                    ...
                    if save_model:
                        ...
                        if args.save_quantized_model:
                            ...
                            quant_model_state_dict = save_quantized_state_dict(quant_model.state_dict(), './pack_pytorch_model.bin')
                            ...
                            quant_model = torch.load(output_model_file)

                            quant_param_size = 0
                            quant_buffer_size = 0
                            for key, val in quant_model.items():
                                 quant_param_size += val.nelement() * val.element_size()

                            # for buffer in quant_model.buffers():
                            #    quant_buffer_size += buffer.nelement() * buffer.element_size()

                            quant_size_all_mb = (quant_param_size) / 1024**2
                            print()
                            print('Teacher Model Size: {:.3f} MB'.format(teacher_size_all_mb))
                            # print('Quantized Model Size: {:.3f} MB'.format(quant_size_all_mb))
                            print('Quantized Model Size: {:.3f} MB'.format(quant_size_all_mb))
                            print()
                            """
            ...
            if any(task_name == task_type for task_type in task_type1):
                acc = result["acc"]    
                student_evaluation_log_df.loc[epoch_] = [epoch_ + 1, elapsed_str, acc, 
                                                         cls_loss, value_loss, query_loss,
                                                         key_loss, rep_loss, loss]
            
            elif any(task_name == task_type for task_type in task_type2):        
                mcc = result["mcc"]
                student_evaluation_log_df.loc[epoch_] = [epoch_ + 1, elapsed_str, mcc, 
                                                         cls_loss, value_loss, query_loss,
                                                         key_loss, rep_loss, loss]
                ...
                f1 = result["f1"]
                acc_and_f1 = result["acc_and_f1"]
                student_evaluation_log_df.loc[epoch_] = [epoch_ + 1, elapsed_str, acc, f1, acc_and_f1,
                                                         cls_loss, value_loss, query_loss,
                                                         key_loss, rep_loss, loss]
                ...
                pearson_and_spearmanr = result["pearson_and_spearmanr"]
                corr = result["corr"]
                student_evaluation_log_df.loc[epoch_] = [epoch_ + 1, elapsed_str, pearson, spearmanr, 
                                                        pearson_and_spearmanr, corr,
                                                         cls_loss, value_loss, query_loss,
                                                         key_loss, rep_loss, loss]      
                          
    student_evaluation_log_df.to_excel(args.log_dir + "_student_evaluation_log.xlsx", index=False)           
    # (revised) 
